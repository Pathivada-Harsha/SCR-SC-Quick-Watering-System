...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['user_id'], $_POST['permissions'])) {
   $userId = filter_input(INPUT_POST, 'user_id', FILTER_SANITIZE_NUMBER_INT);   // Sanitize the user ID
    ...
    // Check if user permissions exist for this user
        $checkQuery = "SELECT COUNT(*) FROM menu_permissions_list WHERE login_id = ?";
    ...
    // If permissions exist, update them, otherwise insert new ones
        ...
        // Build the dynamic UPDATE query
            ...
            $updateQuery = "UPDATE menu_permissions_list SET " . implode(', ', $updateParts) . " WHERE login_id = ?";
        ...
        $insertQuery = "INSERT INTO menu_permissions_list (login_id, $fields) VALUES (?, $placeholders)";
...
function sanitize_permissions($permissions, $conn) {
    ...
    $validFields = ['device_dashboard','dashboard', 'devices_list', 'onoff_control', 'gis_map', 'data_report', 'motor_performance_monitor','energy_consumption','thresholdsettings', 'group_creation', 'location_update', 'notification_settings', 'iotsettings', 'pending_actions','add_new_electrician_devices', 'phase_alerts', 'alerts', 'notification_mesages', 'graphs', 'up_down_time', 'glowing_time', 'user_activity', 'download', 'complaints', 'office_use', 'users_list'];
